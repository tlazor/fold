...
def get_langs_with_wals_feature(feature_id):
    ...
    langs = [lang for lang in get_langs(use_bible=True) if langcodes.Language.get(lang).to_alpha3() in lang_to_value]
...
def analyze_spectra_by_feature_value(output, langs, lang_to_value, plot_dir, feature_id):
...
def main():
    ...
    wals_features = ["17A", "61A", "3A", "12A", "26A", "27A", "34A", "51A", "78A", "64A"]
    ...
    for feature_id in wals_features:
        ...
        analyze_spectra_by_feature_value(output, langs, lang_to_value, plot_dir, feature_id)
